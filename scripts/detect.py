...
from utils.image_utils import (
    load_image, save_image, display_images_grid
)


def demo_line_detection(image, output_dir='results'):
    """
    Demonstrate line detection
    
    Args:
        image: Input image (must be provided)
        output_dir: Directory to save results
    """
    print("\n=== Line Detection Demo ===")
    
    if image is None:
        print("Error: No input image provided for line detection.")
        return None, None
...
def demo_sign_detection(image, output_dir='results'):
    """
    ...
    if image is None:
        print("Error: No input image provided for sign detection.")
        return None
...
def main():
    ...
    if len(sys.argv) <= 1:
        print("Error: No input image provided. Please provide an image path as a command-line argument.")
        sys.exit(1)
    ...
    if input_image is None:
        print("Error: Failed to load image. Please check the provided image path.")
        sys.exit(1)
